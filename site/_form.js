...
/* Campos de contagem (I/O, acionamentos, dispositivos de seguranca): so digito.
   Antes eram texto livre e o cliente escrevia "uns 20" ou "10 a 15"; o servidor
   entao extraia digitos e "10 a 15" virava 1015, calado. Aqui a mascara nao
   deixa nem digitar letra — o campo so aceita numero, e o servidor revalida. */
function mascaraNumero(el) {
...
function notificar(intencao) {
  ...
  /* CAMPOS, nunca o texto pronto. O servidor e quem monta a mensagem.
     Ate 02/08/2026 esta funcao mandava `texto: montar(true)` — o corpo do
     email inteiro escrito no navegador. Quem soubesse disso mandava o que
     quisesse pra caixa do Mateus, e junto com `para_cliente` conseguia fazer
     o site escrever pra terceiros em nome do dominio dele. O navegador nao
     decide mais nada: ele coleta e envia. */
  ...
  /* rede lenta nao pode prender o cliente numa tela de "enviando" pra sempre;
     ...
     caminhos */
  var estouro = new Promise(function (r) {
    setTimeout(function () { r({ ok: false }); }, 12000);
  });
...
function solicitar(botao) {
  ...
  notificar('site').then(function (r) {
    /* O servidor RECUSOU e disse por que: fica na pagina e mostra o motivo.
       Mandar pra tela de "recebido" nesse caso e mentir — o cliente vai
       embora achando que pediu, e o pedido nao existe em lugar nenhum.
       Isso passou despercebido enquanto o servidor aceitava qualquer coisa;
       agora que ele valida de verdade, virou o caminho mais provavel de
       perder um lead. */
    if (!r.ok && r.erro) {
    ...
    /* Sem resposta do servidor — rede caiu, tempo estourou. Ai vale ir pra
       confirmacao, que oferece o WhatsApp como segundo caminho. */
    guardarEnvio(r.ok ? 'ok' : 'falhou');
    if (!r.ok) {
      botao.disabled = false;
      botao.textContent = rotulo;
    }
